...
class VideoAnalyzer:
    ...
    def _analyze(self, vid_path, rp_path, start, end):
        ...
        rp_file = FileRP(rp_path)

        if end is None or end > video.total_frames: end = video.total_frames
        ...
        live_crop = rp_file.master_crop
        live_crop[0] += rp_file.translation[0]
        live_crop[1] += rp_file.translation[1]

        video.config(start, end, live_crop)
        video.open_stream()
